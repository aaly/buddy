...
app_main()
{
  ...
  Services::ServiceRegistry& serviceManager =
    Services::ServiceRegistry::GetInstance();
  // serviceManager.RegisterService<Services::Bluetooth>("bluetooth scanner",
  // "{\"mode\":\"scanner\"}");
  ...
  serviceManager.RegisterEvent(
    Services::Event("display.setbrightness", std::string("{\"level\": 100}")));

  {
    Lib::Timer danceTimer(
      "dance",
      5000,
      [&]() {
        serviceManager.RegisterEvent(Services::Event(
          "rover.dance", std::string("{\"angle\": 220, \"speed\": 100}")));
        delay(1000);
      },
      false);
