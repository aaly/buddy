...
public:
  /**
   * @brief parameters passed around to callbacks using this class, to reduce
   * json parsing to one time only...
   */
  ...
  T GetParameter(const std::string& parameter) const
  {
    ...
    if (_parameters.contains(parameter)) { // Use .contains() for readability
      result = _parameters.at(parameter)
                 .get<T>(); // Use .at() to avoid implicit insertions
    }
    return result;
  };
