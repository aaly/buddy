...
Service::Service(const std::string& name,
  ...
  , _ev_loop_thread(_name, std::bind(&Service::Run, this), _stackSize)
{
...
void
Service::Shutdown(const Services::ParametersPack& parameters)
{
  _keepRunning = false;
}

Result<std::string>
Service::Dispatch(const Event& event)
{
  Serial.printf(
    "Dispatching event [%s] to [%s]\n", event.Name().c_str(), _name.c_str());
  Serial.flush();
  ...
  while (_keepRunning) {
    ...
    if (recv_result.Status() == StatusType::SUCCESS) {
      ...
      if (_event_handlers.find(eventName) != _event_handlers.end()) {
        ...
        ESP_LOGW(
          "Service", "No handler found for event: %s", eventName.c_str());
      }
    } else {
      // TODO report error
    }
...
Result<std::string>
Service::Emit(const Event& e)
{
  Result<std::string> result = _service_manager->RegisterEvent(e);
  if (result.Status() != StatusType::SUCCESS) {
    result.Failure("Could not emit event [" + e.Name() + "] from service [" +
                   _name + "]");
  }
  return result;
}
...
Result<std::string>
Service::Subscribe(const std::string& event_name)
{
  const Event e("subscribe",
                "{\"event\":\"" + event_name + "\", \"service\":\"" + _name +
                  "\"}");
  return Emit(e);
}
} // namespace Services