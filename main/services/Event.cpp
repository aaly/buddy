...
Event::Event(const std::string& name, const std::string& parameters)
  : Event()
{
  std::fill(_eventName, _eventName + _eventNameSize, '\0');
  std::fill(_eventParameters, _eventParameters + _eventParametersSize, '\0');
...
ParametersPack::ParametersPack(const std::string& parameters)
  : _parameters(Parse(parameters)) {};

ParametersPack::ParametersPack(const char* parameters)
  : _parameters(Parse(std::string(parameters))) {};

bool
ParametersPack::HasParameter(const std::string& parameter) const
{
  bool result = false;
...
std::string
ParametersPack::AsString() const
{
  return _parameters.dump();
};

nlohmann::json
ParametersPack::Parse(const std::string& json)
{
