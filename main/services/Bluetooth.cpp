...
void
Bluetooth::Init(const ParametersPack& config)
{
...
void
Bluetooth::handleScan(const Services::ParametersPack& parameters)
{
  ...
  if (scan_result.Status() == StatusType::SUCCESS) {
    for (const auto& connection : scan_result.Data()) {
      ...
      Serial.flush();
      Emit(Event("bluetooth.scanner.results",
                 "{\"name\":\"" + connection.name +
                   "\", \"rssi\":" + std::to_string(connection.rssi) + "}"));
    }
  }
}

Result<std::vector<Connection>>
Bluetooth::Scan(const uint32_t scanTime, const std::string& device_name)
{
  ...
  Lib::Timer bluetoothScannerStopper(
    "bluetoothScannerStopper", scanTime, [&]() { scan->stop(); }, true);
...
// #define GPIO_DEEP_SLEEP_DURATION_SECOND    uS_PER_SEC // sleep x seconds and
// then wake up #define GPIO_DEEP_SLEEP_DURATION_MILLISECOND uS_PER_SEC/1000 //
// sleep x milliseconds and then wake up

// RTC_DATA_ATTR static time_t last;                // remember last boot in RTC
// Memory RTC_DATA_ATTR static uint32_t bootcount; // remember number of boots
// in RTC Memory
...
// #define BEACON_UUID "87b99b2c-90fd-11e9-bc42-526af7764f64" // UUID 1 128-Bit
// (may use linux tool uuidgen or random numbers via
// https://www.uuidgenerator.net/)

// void setBeacon() {

//         BLEBeacon oBeacon = BLEBeacon();
//         oBeacon.setManufacturerId(0x4C00); // fake Apple 0x004C LSB
//         (ENDIAN_CHANGE_U16!) oBeacon.setProximityUUID(BLEUUID(BEACON_UUID));
...
//         Serial.printf("deep sleep (%lds since last reset, %lds since last
//         boot)\n", now.tv_sec, now.tv_sec - last); last = now.tv_sec;
...
//         esp_sleep_enable_timer_wakeup(GPIO_DEEP_SLEEP_DURATION_MILLISECOND *
//         10); Serial.printf("enter light sleep\n");

//         /* To make sure the complete line is printed before entering sleep
//         mode,
...
//         printf("Returned from light sleep, reason: %s, t=%lld ms, slept for
//         %lld ms\n",
//                         wakeup_reason, t_after_us / 1000, (t_after_us -
//                         t_before_us) / 1000);
