...
Result<std::string>
ServiceRegistry::RegisterEvent(const Services::Event& event)
{
...
ServiceRegistry::Run()
{
  while (_keepRunning) {
    ...
    // ESP.getHeapSize(), ESP.getFreeHeap()); Serial.printf("SPIRam Total heap
    // %d, SPIRam Free Heap %d\n", ESP.getPsramSize(), ESP.getFreePsram());
    // Serial.printf("ChipRevision %d, Cpu Freq %d, SDK Version
    // %s\n",ESP.getChipRevision(), ESP.getCpuFreqMHz(), ESP.getSdkVersion());
    // Serial.printf("Flash Size %d, Flash Speed %d\n",ESP.getFlashChipSize(),
    // ESP.getFlashChipSpeed()); Serial.flush();

    Result<Event> recv_result = _events.Receive();
    if (recv_result.Status() == StatusType::SUCCESS) {
      const Event e = recv_result.Data();
      if ("subscribe" == e.Name()) {
        _subscriptions[e.Parameters().GetParameter<std::string>("event")]
                      [e.Parameters().GetParameter<std::string>("service")] =
                        true;
      } else {
