...
public:
  ...
  /**
   * @brief Run the service, this will not start it's own thread/task, not my
   * responsibility bro!
   ...
   * a callback will be registered by the service, and it does send and receive
   * json objects :s, this is not embedded anymore (or is it?)
   */
  std::function<Result<std::string>(const ParametersPack&)> operator[](
  ...
  void On(const std::string& event_name,
          void (T::*handler)(const ParametersPack&))
  {
    ...
    _event_handlers[event_name] =
      [this, handler](const ParametersPack& params) -> void {
...
protected:
  ...
  std::unordered_map<std::string,
                     std::function<void(const Services::ParametersPack&)>>
    _event_handlers;
