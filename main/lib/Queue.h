...
public:
  ...
  Queue() { _queue = xQueueCreate(_queueSize, sizeof(T)); }

  /**
   * @brief receive a message, if queue und block forever until a messga
   * earrives
  ...
  Result<T> Receive(const QueueDelayType& delay = QueueDelayType::MAX)
  {
    ...
    T message;
    auto recv_result =
      xQueueReceive(_queue, &message, static_cast<TickType_t>(delay));
   ...
   */
  Result<std::string> Send(const T message,
                           const QueueDelayType& wait = QueueDelayType::MAX)
  {
    Result<std::string> result;
    auto send_result =
      xQueueSend(_queue, &message, static_cast<TickType_t>(wait));
