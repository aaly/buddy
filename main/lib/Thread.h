...
{
public:
  Thread(const std::string& taskName,
  ...
  /**
   * @brief Run the task/thread, return result with a status message containing
   * task name
   */
  ...
  /**
   * @brief method adapter to be called by Run() and accomodate the xTaskCreate
   * C interface
   */
  static void methodAdapter(void* pvParameters)
  {
    std::function<void(void)> callback =
      *static_cast<std::function<void(void)>*>(pvParameters);
    callback();
  }
