...
public:
  Result() {};
  Result(const StatusType& status, const T& data)
    : _status(status)
    , _data(data) {};

  inline void SetData(const T& data) { _data = data; }

  inline void SetStatus(StatusType status) { _status = status; }
  ...
  bool Succeded() const { return StatusType::SUCCESS == _status; };
