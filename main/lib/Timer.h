...
public:
  ...
  Timer(const Timer& timer)
    : _timerName(timer.GetName())
    , _period(timer.GetPeriod())
    , _callback(std::move(timer.GetCallback()))
    , _singleShot(timer.GetSingleShot())
  {
  }
  ...
  Result<std::string> Start()
  {
    ...
    if (nullptr != _callback) {
      ...
      _timerHandle = xTimerCreate(
        ...
        pdMS_TO_TICKS(
          _period), /* The timer period in ticks, must be greater than 0. */
        _singleShot ? pdFALSE : pdTRUE, /* The timers will auto-reload
                                           themselves when they expire. */
        (void*)_callbackWrapper, /* The ID is used to store a count of the
  ...
  std::string GetName() const { return _timerName; }

  uint32_t GetPeriod() const { return _period; }

  bool GetSingleShot() const { return _singleShot; }

  std::function<void(void)> GetCallback() const { return _callback; }
